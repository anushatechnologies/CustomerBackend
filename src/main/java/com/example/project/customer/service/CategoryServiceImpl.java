...
public class CategoryServiceImpl implements CategoryService {
    ...
    public CategoryResponse getById(Integer id) { return response(find(id)); }

    @Transactional(readOnly = true)
    public List<CategoryResponse> getAll() { return repository.findAll().stream().map(this::response).toList(); }
    ...
    private Category find(Integer id) { return repository.findById(id).orElseThrow(() ->
            new ResourceNotFoundException("Category not found with id: " + id)); }
    private ResourceConflictException conflict(String slug) { return new ResourceConflictException("Category already exists with slug: " + slug); }
    private Category apply(Category category, CategoryRequest request) {
        category.setName(request.name()); category.setSlug(request.slug()); category.setImageUrl(request.imageUrl());
        category.setActive(request.active()); category.setSortOrder(request.sortOrder()); return category;
    }
    private CategoryResponse response(Category c) { return new CategoryResponse(c.getCategoryId(), c.getName(), c.getSlug(), c.getImageUrl(), c.isActive(), c.getSortOrder(), c.getCreatedAt()); }
}