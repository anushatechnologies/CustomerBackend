...
public class SubcategoryServiceImpl implements SubcategoryService {
    ...
    public SubcategoryResponse getById(Integer id) { return response(find(id)); }
    @Transactional(readOnly = true)
    public List<SubcategoryResponse> getAll() { return repository.findAll().stream().map(this::response).toList(); }
    ...
    private Subcategory find(Integer id) { return repository.findById(id).orElseThrow(() -> new ResourceNotFoundException("Subcategory not found with id: " + id)); }
    private ResourceConflictException conflict(String slug) { return new ResourceConflictException("Subcategory already exists with slug: " + slug); }
    private Subcategory apply(Subcategory item, SubcategoryRequest request) {
        item.setCategory(categoryRepository.findById(request.categoryId()).orElseThrow(() -> new ResourceNotFoundException("Category not found with id: " + request.categoryId())));
        item.setName(request.name()); item.setSlug(request.slug()); item.setImageUrl(request.imageUrl()); item.setActive(request.active()); item.setSortOrder(request.sortOrder()); return item;
    }
    private SubcategoryResponse response(Subcategory s) { return new SubcategoryResponse(s.getSubcategoryId(), s.getCategory().getCategoryId(), s.getName(), s.getSlug(), s.getImageUrl(), s.isActive(), s.getSortOrder(), s.getCreatedAt()); }
}